...
provider-swappable via env vars) produces detection and root-cause
verdicts stored on `run_metadata`. The
...
OpenAI-compatible endpoint; provider swap = `ANALYSIS_BASE_URL`/
`ANALYSIS_API_KEY`/`ANALYSIS_MODEL` env vars only. Forbidden: LangChain,
...
| G10 | Fixed stack | FastAPI/SQLModel/NeonDB/Next.js/Tailwind/httpx/Gemma 4 (via OpenRouter, provider-swappable by env vars) exactly as pinned; nothing substituted | PASS |

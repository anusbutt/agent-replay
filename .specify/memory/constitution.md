...
- Stack is fixed (do not substitute): Python 3.11+ SDK with httpx; FastAPI +
  ...
  (AMD compute demonstration lives separately in `amd/` — ROCm/PyTorch
  brought up on an AMD Radeon gfx1100 with verified GPU compute; the
  full-prompt run script is committed there). No LangChain, no LangGraph, no
